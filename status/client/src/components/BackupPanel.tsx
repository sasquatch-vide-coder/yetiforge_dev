...
export function BackupPanel({ token }: Props) {
  ...
  return (
    <div className="bg-brutal-white brutal-border brutal-shadow p-6">
      <h2 className="text-xl font-bold uppercase mb-4 font-mono">Backups</h2>

      {errorMsg && (
        <p className="font-mono text-sm text-brutal-red mb-2">{errorMsg}</p>
      )}
      {statusMsg && (
        <p className="font-mono text-sm text-brutal-green font-bold mb-2">{statusMsg}</p>
      )}
      ...
      {loading ? (
        <p className="text-sm font-mono text-brutal-black/50 uppercase">Loading backups...</p>
      ) : backups.length === 0 ? (
          ...
          <table className="w-full text-sm font-mono">
            ...
            <tbody>
              {backups.map((backup, i) => (
                ...
                >
                  <td className="p-2">{formatTime(backup.timestamp)}</td>
                  <td className="p-2">{backup.files}</td>
                  <td className="p-2">{formatBytes(backup.sizeBytes)}</td>
                  <td className="p-2">
                    <div className="flex gap-2">
                      <button
                        onClick={() => handleRestore(backup.id)}
                        className="bg-brutal-blue text-brutal-white font-bold uppercase py-1 px-2 brutal-border text-xs font-mono hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none brutal-shadow transition-all"
                      >
                        Restore
                      </button>
                      <button
                        onClick={() => handleDelete(backup.id)}
                        className="bg-brutal-red text-brutal-white font-bold uppercase py-1 px-2 brutal-border text-xs font-mono hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none brutal-shadow transition-all"
                      >
                        Delete
                      </button>
                    </div>
